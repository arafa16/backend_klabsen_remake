...
const getCountDatas = async (req, res) => {
  const { periode_uuid, group_uuid, status_uuid, user_uuid } = req.query;
  ...
  if (periode_uuid) {
    ...
    if (findPeriode !== null) {
      const startDate = date.format(
        new Date(findPeriode.tanggal_mulai),
        "YYYY-MM-DD HH:mm:ss",
      );
      const endDate = date.format(
        new Date(findPeriode.tanggal_selesai),
        "YYYY-MM-DD HH:mm:ss",
      );
  ...
  try {
    ...
    for (const data in findUser) {
      ...
      for (const dataInOut in getDataInOut) {
        if (
          ...
          if (getDataInOut[dataInOut].pelanggaran.code === 2) {
            data_perhitungan[data].data_in_pelanggaran =
              data_perhitungan[data].data_in_pelanggaran + 1;
          } else {
            data_perhitungan[data].data_in_normal =
              data_perhitungan[data].data_in_normal + 1;
          }
        } else if (
          ...
          if (getDataInOut[dataInOut].pelanggaran.code === 2) {
            data_perhitungan[data].data_out_pelanggaran =
              data_perhitungan[data].data_out_pelanggaran + 1;
          } else {
            data_perhitungan[data].data_out_normal =
              data_perhitungan[data].data_out_normal + 1;
          }
        } else if (getDataInOut[dataInOut].tipe_absen.code === 13) {
          data_perhitungan[data].data_cuti =
            data_perhitungan[data].data_cuti + 1;
        } else if (getDataInOut[dataInOut].tipe_absen.code === 14) {
          data_perhitungan[data].data_sakit =
            data_perhitungan[data].data_sakit + 1;
        } else {
          data_perhitungan[data].data_tidak_absen =
            data_perhitungan[data].data_tidak_absen + 1;
        }
      }
      ...
      const tidak_absen =
        queryDataPeriode.jumlah_hari -
        ((data_perhitungan[data].data_in + data_perhitungan[data].data_out) /
          2 +
          data_perhitungan[data].data_cuti +
          data_perhitungan[data].data_sakit);
      ...
      if (tidak_absen < 0) {
        data_perhitungan[data].data_tidak_absen =
          data_perhitungan[data].data_tidak_absen + 0;
      } else {
        data_perhitungan[data].data_tidak_absen =
          data_perhitungan[data].data_tidak_absen + tidak_absen;
      }

      data_perhitungan[data].total_point =
        data_perhitungan[data].data_in_pelanggaran +
        data_perhitungan[data].data_out_pelanggaran +
        data_perhitungan[data].data_tidak_absen;

      data_perhitungan[data].point_pelanggaran =
        data_perhitungan[data].total_point * 0.005;
    }
