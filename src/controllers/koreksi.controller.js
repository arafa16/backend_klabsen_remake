...
const updateData = async (req, res) => {
  const { user_id, in_out_id, keterangan, status_koreksi_id, is_active } =
    req.body;
...
const approvalData = async (req, res) => {
  ...
  try {
    const result = await findKoreksi.update({
      status_koreksi_id: findStatusKoreksi.id,
    });

    const findInOut = await inOutModel.findOne({
      where: {
        id: findKoreksi.in_out_id,
      },
    });

    if (findStatusKoreksi.code === 2 || findStatusKoreksi.code === "2") {
      const findStatusInout = await statusInoutModel.findOne({
        where: {
          code: 4,
        },
      ...
      await findInOut.update({
        status_inout_id: findStatusInout.id,
        pelanggaran_id: 1,
      });
    } else if (findStatusKoreksi.code === 3 || findStatusKoreksi.code === "3") {
      const findStatusInout = await statusInoutModel.findOne({
        where: {
          code: 3,
        },
