...
const getDataByUser = async (req, res) => {
  ...
  const findUser = await userModel.findOne({
    ...
    attributes: ["id", "uuid", "name", "email", "privilege_id"],
    include: [
      {
  ...
  if (tahun) {
    queryObjectYear = sequelize.where(
      sequelize.fn("YEAR", sequelize.col("tanggal_mulai")),
      tahun,
    );
...
const getDataCount = async (req, res) => {
  ...
  const start_date = date.format(
    new Date(tanggal_mulai),
    "YYYY-MM-DD HH:mm:ss",
  );
  const end_date = date.format(
...
const createData = async (req, res) => {
  ...
  //moment
  const tanggal_mulai_moment = moment(tanggal_mulai).format(
    "YYYY-MM-DD HH:mm:ss",
  );
  const tanggal_selesai_moment = moment(tanggal_selesai).format(
    "YYYY-MM-DD HH:mm:ss",
  );
